...
export class FeatureStore {
  ...
  related(id: string, limit = 20): { sharedPathway: FeatureSummary[]; sharedFunction: FeatureSummary[] } {
    const base = this.find(id);
    if (!base) return { sharedPathway: [], sharedFunction: [] };

    const pwTags = new Set<string>([...base.KG_PW, ...base.UP_PW].map((s) => s.toLowerCase()));
    const fnTags = new Set<string>(
      [...base.KG_FG, ...base.KG_FM, ...base.UP_KW].map((s) => s.toLowerCase())
    );

    const sharedPathway: FeatureSummary[] = [];
    const sharedFunction: FeatureSummary[] = [];
    for (const f of this.all) {
      if (f.uniqID === base.uniqID) continue;
      const fPw = [...f.KG_PW, ...f.UP_PW].map((s) => s.toLowerCase());
      const fFn = [...f.KG_FG, ...f.KG_FM, ...f.UP_KW].map((s) => s.toLowerCase());
      if (sharedPathway.length < limit && fPw.some((t) => pwTags.has(t))) sharedPathway.push(toSummary(f));
      if (sharedFunction.length < limit && fFn.some((t) => fnTags.has(t))) sharedFunction.push(toSummary(f));
      if (sharedPathway.length >= limit && sharedFunction.length >= limit) break;
    }
    return { sharedPathway, sharedFunction };
  }
}
