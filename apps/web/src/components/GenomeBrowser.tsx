...
// Tableau 10 — distinguishable, neutral semantics.
const CAT_PALETTE = [
  '#4e79a7',
  '#f28e2c',
  '#e15759',
...
export default function GenomeBrowser({
  ...
  const plusY = RULER_HEIGHT;
  const minusY = RULER_HEIGHT + TRACK_HEIGHT + STRAND_GAP;
  const totalHeight = RULER_HEIGHT + TRACK_HEIGHT * 2 + STRAND_GAP + PADDING;

  // On a circular molecule there's no hard end to fade toward — content always
  // continues past both edges — so the edge fades only apply to linear chromosomes.
  const fadeLeft = !isCircular && view.from > 1;
  const fadeRight = !isCircular && view.to < chromosome.length;
  ...
  function renderFeatureRow(f: FeatureSummary, rowY: number) {
    ...
    const onClick = interactive
      ? () => {
          if (selected?.uniqID === f.uniqID) {
            nav(`/o/${taxid}/c/${encodeURIComponent(f.chrom)}`);
          } else {
            nav(`/o/${taxid}/c/${encodeURIComponent(f.chrom)}/entry/${f.uniqID}`);
          }
        }
      : undefined;
    return (
  ...
  return (
    <div className="space-y-2">
      ...
      <div ref={containerRef} className="relative w-full pt-2">
        <svg
          ref={svgRef}
          width={width}
          height={totalHeight}
          ...
          className="block w-full select-none rounded border border-neutral-200 bg-white"
        ...
        >
          <g transform={`translate(${PADDING},0)`}>
            ...
            <line x1={0} x2={innerWidth} y1={RULER_HEIGHT - 4} y2={RULER_HEIGHT - 4} stroke="#d4d4d4" />
            <line x1={0} x2={innerWidth} y1={plusY + TRACK_HEIGHT / 2} y2={plusY + TRACK_HEIGHT / 2} stroke="#e5e5e5" />
            <line x1={0} x2={innerWidth} y1={minusY + TRACK_HEIGHT / 2} y2={minusY + TRACK_HEIGHT / 2} stroke="#e5e5e5" />
            {!animFreeze && (
              <>
                <text x={-6} y={plusY + TRACK_HEIGHT / 2 + 3} fontSize={10} fill="#737373" textAnchor="end">+</text>
                <text x={-6} y={minusY + TRACK_HEIGHT / 2 + 3} fontSize={10} fill="#737373" textAnchor="end">−</text>
              </>
            )}
            ...
            >
              {selected?.coord && (() => {
                ...
                return (
                  <rect
                    ...
                    opacity={0.18}
                    onClick={() => nav(`/o/${taxid}/c/${encodeURIComponent(selected.chrom)}`)}
              ...
              {ticks.map((t) => (
                <g key={t} transform={`translate(${bpToPx(t)},0)`}>
                  <line y1={RULER_HEIGHT - 8} y2={RULER_HEIGHT - 4} stroke="#a3a3a3" />
                  {!animFreeze && (
                    <text y={RULER_HEIGHT - 10} fontSize={10} fill="#525252" textAnchor="middle">
              ...
              {isCircular && renderView.to > L && (
                <g transform={`translate(${bpToPx(L + 0.5)},0)`}>
                  ...
                  />
                  {!animFreeze && (
                    <text y={RULER_HEIGHT - 10} fontSize={9} fill="#737373" textAnchor="middle">
                      ori
                    </text>
                  )}
                </g>
    ...
    >
      ...
      >
        ...
        <rect
          ...
          width={W}
          height={BAR_H}
          fill={active ? '#f5f5f5' : '#fafafa'}
          stroke={active ? '#d4d4d4' : '#e5e5e5'}
        />
        {pieces.map((p, i) => (
          <rect
            key={i}
